...
/** Fit a slot interval inside the event window, preserving duration when possible. */
export function clampSlotIntervalToWindow(
  ...
  if (!Number.isFinite(ws) || !Number.isFinite(we) || we <= ws || durationMs <= 0) return null

  const windowSpan = we - ws
  const dur = Math.min(durationMs, windowSpan)
  let startMs = proposedStartMs
  ...
  if (startMs < ws) startMs = ws
  if (endMs > we) endMs = we
