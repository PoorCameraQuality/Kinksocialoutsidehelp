...
export default function PresenterEckePanel({ presenterUserId }: Props) {
  ...
  const loadStatus = useCallback(async () => {
    ...
    if (r.status === 403) {
      setStatusData(null)
      setAccessError(
        ECKE_PANEL_ACCESS_MESSAGES.presenter_profile ??
          'Only the presenter owner can publish this presenter profile to ECKE.',
      )
      return
    }
    if (!r.ok) {
  ...
  return (
    <EckePublishPanel
      ...
      publishRestrictedMessage={
        !canPublish ? ECKE_PUBLISH_RESTRICTED_MESSAGES.presenter_profile : undefined
      }
